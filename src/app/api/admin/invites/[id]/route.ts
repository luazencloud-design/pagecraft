...
import { renameInvite, regenerateInvite, deleteInvite, inviteLink, getInvite } from '@/lib/invites'

/** 수정 — 이름 변경(rename) 또는 링크 재생성(regenerate) */
export async function PATCH(req: Request, ctx: { params: Promise<{ id: string }> }) {
  const admin = await requireAdmin()
  if (!admin) return NextResponse.json({ error: '관리자 권한 필요' }, { status: 403 })

  const { id } = await ctx.params
  const body = (await req.json().catch(() => ({}))) as { action?: 'rename' | 'regenerate'; name?: string }
