...
## #22. 배경 제거: Gemini → Replicate Recraft 전환

**배경**
- #15 에서 Gemini 배경제거의 한계를 기록하고 단기 대응(프롬프트 강화 + `whitenNearWhite` 후처리)만 적용한 상태였음
- 실제 사용자 피드백에서도 "상품 변형" / "회색 배경" 이슈가 지속적으로 보고됨

**품질 + 비용 비교 (전환 판단 근거)**

| 항목 | Gemini 2.5 Flash Image | **Replicate Recraft** | BRIA RMBG 2.0 | @imgly WASM |
|------|------------------------|----------------------|---------------|-------------|
| 방식 | 생성형 (재생성) | **진짜 픽셀 마스크** | 진짜 픽셀 마스크 | 클라이언트 WASM |
| 상품 변형 위험 | 있음 (pose/개수) | **없음** | 없음 | 없음 |
| 출력 | 흰 배경 근사 (회색빛) | **투명 PNG (alpha)** | 투명 PNG | 투명 PNG |
| 후처리 필요 | `whitenNearWhite` 필수 | 불필요 | 불필요 | 불필요 |
| 비용 / 건 | ~$0.04 | **$0.01** | ~$0.058 | 무료 |
| 처리 시간 | 10~30s (503 잦음) | 3~10s | 3~8s | 수초 (모바일 10s+) |
| 초기 비용 | 0 | 0 | 0 | 첫 로드 40MB |

→ **Recraft가 품질/비용 모두 우위**. BRIA는 오버스펙, @imgly는 데모 UX(첫 로드 40MB) 와 안 맞음.

**구현**
```ts
// src/services/ai.service.ts (요지)
const output = await replicate.run(
  'recraft-ai/recraft-remove-background',
  { input: { image: imageDataUrl } }
)
// output.url() → 서버 fetch → base64 변환 → 반환 (클라 API 계약 유지)
```

- Replicate Node SDK는 data URL 직접 수용 (내부 자동 업로드)
- SDK 버전에 따라 `FileOutput` 객체 / 문자열 / 배열 가능성 → 방어적 분기
- 결과 URL 은 ~1시간 후 만료하지만, 서버가 즉시 fetch → base64 로 변환해서 IndexedDB 에 영구 저장
- **CORS 우회** 도 같이 해결 (Replicate CDN의 CORS 정책 의존 제거)

**Gemini 코드 주석 보존**
- `removeBackgroundGemini` 는 블록 주석(`/* ... */`) 으로 보존
- 복귀 조건: Replicate 장애 / 요금 폭등 / Gemini 에 배경제거 전용 모델 등장
- 블록 주석은 TypeScript 컴파일러가 건드리지 않아 dead-code 부담 없음

**코드 변경 영향**
- 클라 코드 **0줄 변경** — `/api/image/bg-remove` 응답 포맷(`{ image: 'data:image/png;base64,...' }`) 유지
- `whitenNearWhite` 후처리는 호출되지만 투명 픽셀(alpha=0)은 처리 대상 아니라 **무해**. 추후 정리 대상 (known-issues 기술부채 목록)

**Vercel 환경 제약 검증 (#21 참조)**
- 요청 payload: 1024px JPEG × 0.9 ≈ 400KB (4.5MB 여유)
- 응답 payload: 투명 PNG 1024px ≈ 1~2MB + base64 1.33× = 최대 ~2.7MB (4.5MB 여유)
- maxDuration: Recraft 3~10s × `maxDuration=60` 여유
- cold start: Replicate SDK ~50ms 초기화, 모듈 싱글톤으로 재사용

**비용 절감**
- 배경제거 월 100건 × $0.04 = $4 → × $0.01 = **$1** (75% 감소)
- 최악 시나리오(월 150명 × 100% 사용률): ~82만원 → ~45만원

**관련 파일**
- `src/services/ai.service.ts` — `removeBackgroundRecraft` 추가, Gemini 버전 주석 보존
- `src/app/api/image/bg-remove/route.ts` — import 교체
- `.env.example`, `.env.local` — `REPLICATE_API_TOKEN` 추가
...
## #25. 만료된 초대: 자동 삭제 → 목록 유지('만료' 표시)

**문제**: 기존엔 `getInvite`가 만료된 초대를 **조회 시 자동 삭제(lazy)** 했음. 관리자가 모르는 새 목록에서 사라져 추적·회수 이력이 끊김.

**변경**: 만료돼도 **레코드 유지**, 관리자가 직접 삭제.
- `getInvite` — 만료 자동삭제 제거. 단순 조회만 (실제 삭제는 `deleteInvite`만)
- 차단은 분리: `aiGate`·`/api/auth/me` 가 `inviteUsableReason(inv) !== 'ok'` 로 막음 (레코드 존재 ≠ 사용 가능)
- `listInvites` — 만료 건도 포함 (인덱스에만 남은 유령 레코드만 정리)
- 관리자 UI — 만료 행은 빨간 테두리 + '만료' 배지, **삭제 버튼 행에 상시 노출**(편집 패널 안 열어도 됨), 만료 행은 링크 복사 숨김
- 부가 이점: 만료된 초대도 **종료일만 미래로 수정하면 재활성화** 가능

**불변**: 만료/삭제 시 사용자 차단은 그대로 즉시(다음 서버 요청). 달라진 건 "레코드가 남느냐"뿐.

---

## #26. 관리자 활동 로그 탭 + 만료 링크 안내 페이지

**로그 관리 개선** (입장 이벤트 폭증 대비)
- 관리자 페이지를 **탭 분리**: `초대 링크` / `활동 로그`
- 로그 탭: **필터 칩**(전체/입장/생성/재생성/삭제, 각 건수 표시) + **스크롤 박스**(maxHeight 460)
- 입장(redeemed)이 많아도 필터로 관리 이벤트(생성/삭제)만 골라볼 수 있음
- 보관량 100 → **300**건, 관리자 조회 30 → 300

**만료/무효 링크 진입 안내** (`/invite-error?reason=`)
- 기존: 만료·삭제·버전불일치 다 같은 `/?invite=revoked` (구분 X, 안내 X)
- 변경: `/api/auth/invite` + OAuth 콜백이 사유 구분 → 전용 안내 페이지
  | reason | 화면 |
  |---|---|
  | `expired` | "초대 기간이 만료되었어요" |
  | `gone` | "존재하지 않는 초대예요" (삭제됨 / 재생성된 옛 링크) |
  | `not_started` | "아직 시작 전인 초대예요" |
...
## #29. 카테고리 대확장 (코스트코 기준) + 2단 드롭다운

**배경**: 카테고리는 AI 생성의 참고자료(텍스트 톤 + 모델샷 프레이밍). 기존 3그룹(의류·잡화/뷰티/유아·아동)으로는 판매 품목 커버 불가 → 코스트코 온라인몰 분류 기준으로 확장.

**카테고리**: 3그룹 25종 → **12그룹 78종**
- 의류 / 패션잡화 / 보석·시계 / 화장품·뷰티 / 유아·아동 / 식품 / 주방·생활 / 가전·디지털 / 가구·인테리어 / 스포츠·레저 / 반려동물 / 문구·공구·차량
- 구 카테고리명('기타 의류/잡화' 등)은 프레이밍 맵에 유지 → 저장된 드래프트 호환

**UI**: 네이티브 `<select>` → **2단 팝오버**(`CategorySelect.tsx`)
- 왼쪽 대분류 호버/클릭 → 오른쪽으로 세부 카테고리 펼침
- 선택된 그룹은 강조 표시, ESC/바깥클릭 닫기, 선택 해제 버튼

**프레이밍**: 신규 카테고리 전부 `byExactCategory`에 매핑 (기존 하네스와 같은 맥락)
| 제품군 | 프레이밍 |
|---|---|
| 착용류(의류/신발/잡화) | 기존 규칙 확장 — 부위별 크롭 (원피스=전신, 니트=상반신 등) |
| 보석·시계 | 착용 부위 클로즈업 (목걸이=쇄골, 반지=손가락, 시계=손목) |
| 식품 | 얼굴 없이 손/테이블 중심 — 제품·음식이 주인공 |
| 주방·생활·가전 | 사용 장면 (주방/거실 등 실사용 컨텍스트) |
| 가구·인테리어 | 와이드 공간 연출 |
| 스포츠·레저 | 전신 활동 장면 |
| 반려동물 | 반려동물 + 제품 (사료는 패키지 중심) |

**신규 F 상수**: handsProduct / tableScene / kitchenScene / lifestyleWide / neckCloseup / earCloseup

**주의**: PR #50(유아·아동) 위에 쌓임 — #50 먼저 머지.

### #29 보완 — 코스트코 분류에 충실하게 재정렬
초기 버전이 코스트코를 참고만 하고 임의로 추려서(디지털/TV, 대형가전, 파티오/정원, 건강·영양제 그룹 누락) 실제 사이트와 달랐음 → **대분류 구조를 코스트코 그대로 정렬**: 12그룹 78종 → **16그룹 129종**.
- 추가된 그룹: 디지털·TV·컴퓨터 / 가전(냉장고·세탁기 등 대형 포함) / 파티오·정원 / 건강·영양제(8종 세분화)
- 추가된 항목: 칼/도마, 겨울 스포츠, 순금/순은, 유아동 가구, 생리대/위생용품, 건식품, 유기농식품 등
- 유일한 의도적 편차: 의류를 남성/여성 대신 **아이템별** 분류 (성별은 AI 모델 토글에 있고, 프레이밍 엔진이 아이템 타입 필요)
- 전 129종 byExactCategory 커버리지 기계 검증 (누락 0)
- modelAgeForCategory에 '어린이' 키워드 추가 (어린이 영양제 → child)

---

## #30. 카테고리 검색 + 이미지 피사체 토글 (모델/제품만)

**카테고리 검색** — 137종이 되면서 눈으로 찾기 불가 → 팝오버 상단 검색창.
- 항목명/그룹명 매칭, 결과는 `그룹 › 항목` 평면 리스트, Enter로 첫 결과 선택
- 검색어 비우면 기존 2단 뷰로 복귀

**피사체 토글** — 이미지 생성 시 `👤 모델 착용 | 📦 제품만` 선택.
- `AIModelImageRequest.subject?: 'model' | 'product'` (생략=model, 기존 호환)
- 제품만: 사람 없이 스튜디오 제품컷 — 기존 각도컷 파이프라인(generateAngleShot) 재사용
  - 단일 생성 = 정면 히어로 컷 / 풀세트 = 각도 컷 N장 전부 (모델 컷 스킵)
- 제품만 모드에선 성별/연령 세그먼트 비활성(dim) — 사람이 없으니
- imageStore.aiModelSubject, 풀세트 안내 문구도 모드별 전환

### #29 재보완 — 코스트코 1:1 완전 미러링 (사용자 요청)
아이템별 의류 분류(의도적 편차)도 제거하고 **코스트코 분류 그대로**: 16그룹 131종.
- 대분류·중분류 이름을 코스트코 표기 그대로 (여성의류/남성의류, 유아동/완구/반려동물 통합 등)
- 프레이밍 95개 신규 생성 (기존 이름과 겹치는 건 스킵 로직으로 중복 키 방지)
- 제외 4개: 기프트카드/사전주문/비즈니스 딜리버리/Costco Direct — 상품이 아닌 판매방식
- 보석 그룹의 '선글라스/안경테'는 의류/가방/잡화 쪽 1곳만 유지 (완전 중복 항목)
- 구(旧) 이름 프레이밍은 byExactCategory에 잔존 → 저장된 드래프트 호환
